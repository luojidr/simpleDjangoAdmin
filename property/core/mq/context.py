...
class ContextBaseTask(Task):
    ...
    def backend(self):
        ...
        task_to_backend = parameters.get(CeleryConfig.CELERY_TASK_TO_BACKEND)
        to_backend = task_to_backend.default if task_to_backend else None
        to_backend = None if to_backend == "default" else to_backend

        if to_backend:
            # 任务结果存储到不同介质
            result_backend = getattr(CeleryConfig, "CELERY_RESULT_BACKEND_" + to_backend.upper(), None)
    ...
    @staticmethod
    def log_info(self=empty, current_running_fun=None, **kwargs):
        if self is empty:
            worker_logger.info(">>>>>> ContextTask.log have't bond task instance !!!")

        kwargs["self_id"] = id(self)
        ...
        log_msg = "{task_cls_name}.{current_running_fun} {log_msg} -> {running_fun_kwargs}".format(
            ...
            current_running_fun=current_running_fun, running_fun_kwargs=kwargs
        )
    ...
    def on_success(self, retval, task_id, args, kwargs):
        ...
        self.log_info(current_running_fun=inspect.stack()[0][3], **log_kwargs)

    def on_retry(self, exc, task_id, args, kwargs, einfo):
        self.log_info(current_running_fun=inspect.stack()[0][3], **dict(locals()))
    ...
    def after_return(self, status, retval, task_id, args, kwargs, einfo):
        ...
        self.log_info(current_running_fun=current_running_fun, **log_kwargs)
        ...
        if status != SUCCESS:
            ...
            self.log_info(current_running_fun=current_running_fun, **retry_log_kwargs)
    ...
    def __call__(self, *args, **kwargs):
        ...
        self.log_info(current_running_fun=inspect.stack()[0][3], **dict(log_kwargs, args=args, kwargs=kwargs))
...
class ContextTask(ContextBaseTask):
    ...
    def _get_raw_app(self):
        ...
        raw_app.config_from_object(obj=CeleryConfig)
    ...
    def delay(self, *args, **kwargs):
        """ 追踪 task 日志 """
        task_logger.info("ContextTask.delay task <%s> send message ==>>> args:%s, kwargs: %s",
    ...
    def apply_async(self, args=None, kwargs=None, task_id=None, producer=None,
                    link=None, link_error=None, shadow=None, **options):
        """ 追踪 task 日志 """
        task_logger.info("ContextTask.apply_async task <%s> send message ==>>> args:%s, kwargs: %s",
                         self.name, args, kwargs)
